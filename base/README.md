...
| Gateway | `sgw-agent` (the AI's command; `sgw-agent setup` at every start), `sekimore-relay`, `sekimore` (the former name), `sgw-post-start` (what `postStartCommand` runs), copied out of `ghcr.io/amakata/sekimore-gw:0.2.50` |
