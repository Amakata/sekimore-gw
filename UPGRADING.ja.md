...
0.2.38 〜 0.2.43 では作業は不要です。[0.2.46](#0246-base-イメージが-sekimore-gw-に移った) も不要で、ファイルの出どころが変わったことを書いているだけです。[0.2.47](#0247-sgw-が使えるようになった) も不要で、mise のタスクの横に `sgw` が増えるだけです。0.2.48 も不要で、0.2.47 で約束したものが揃うだけです。
