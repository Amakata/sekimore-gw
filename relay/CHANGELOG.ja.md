...
- エラーの中の操作者向けコマンドに、関所の中で動かすことを明記した。`sekimore-relay keyscan` を dev で実行した AI が、そこには存在しない設定ファイルを読んで無関係な原因を報告していた
- 上流プロキシへの `sekimore-relay login` のタイムアウトには、この設定のルールではない原因がある。プロキシのアドレスが docker のブリッジサブネットに入っていると、コンテナはそこへ route しない。次の一手として自然に出る allow_ips の拡大では直らないので、起動時にその旨を出す
