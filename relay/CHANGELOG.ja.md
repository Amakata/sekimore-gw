...
- 同じ pack 内の、別のコミットに対するコミットの delta を適用するようにした。`signing: required` で署名つきコミット 2 つの push が通る。判定するのは適用した結果で、土台ではない。このために 1 MiB までのコミットを保持する (pack あたり 64 MiB まで)。保持しなかったコミットに対する delta は引き続き拒否し、案内を効かない `--no-thin` から `git -c pack.window=0 push` に改めた (#144)
