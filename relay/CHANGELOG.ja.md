...
- relay 自体の変更は無い。0.2.47 と 0.2.48 は同じバイナリ。sgw のために、クレートが macOS でコンパイルできるようになった (#244)

## 0.2.47（2026-09-26）

### Enhancement

- relay 自体の変更は無い。0.2.46 と 0.2.47 は同じバイナリ。この版はその横にホスト側の道具 sgw を足す (#238)

## 0.2.46（2026-09-26）

### Enhancement

- relay 自体の変更は無い。0.2.45 と 0.2.46 は同じバイナリ。この版は dev コンテナの base イメージをこのリポジトリに取り込む (#236)
...
- relay 自体の変更は無い。0.2.42 と 0.2.43 は同じバイナリ。この版は gateway の `/api/config` で上流プロキシのパスワードを伏せる (#218)

## 0.2.42（2026-09-25）

### Enhancement

- `check` の `proxy:` の下に `egress:` 行を足した。上流プロキシがあるのに直接送出を許していれば黄、Squid だけが出口なら緑 (#215)
...
- 443 の通過で、`https://` の上流プロキシには CONNECT の前に TLS で接続するようにした。信頼する CA は GitHub クライアントと同じ。API クライアントは以前から TLS プロキシを通れていた (#199)

### Enhancement

- `check` と `store-status` が、上流プロキシの資格情報がストアにあるか、施錠中か、無いかを、次に打つコマンドと一緒に言うようにした (#200)

## 0.2.38（2026-09-25）

### Enhancement

- relay 自体の変更は無い。0.2.37 と 0.2.38 は同じバイナリ。この版は DOCKER-USER の規則に加えて INPUT の規則でホスト自身を dev から閉じる (#191)

## 0.2.37（2026-09-25）

### Enhancement

- relay 自体の変更は無い。0.2.36 と 0.2.37 は同じバイナリ。この版は gateway がホスト側の `DOCKER-USER` に FORWARD 規則を入れてエージェントを閉じ込める (#189)
...
- passthrough と GitHub API のクライアントが、上流 proxy の資格情報を秘密ストアから、接続のたびに読んで使う。解錠や `gw:proxy-credential` での変更が再起動なしに効く。環境変数と `config.yml` は引き続き代わりに使う (#152)
- 上流 proxy が 407 を返したとき、relay が使った資格情報の出どころを伝える。`check` にも出す (#152)
