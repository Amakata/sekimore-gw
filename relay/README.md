...
- There are 28 permission keys: `pr:create` `pr:read` `pr:comment` `pr:review` `pr:request_review` `pr:merge` `pr:close` `pr:label` `pr:assign`, `issue:create` `issue:read` `issue:comment` `issue:update` `issue:close` `issue:label` `issue:assign`, `project:read` `project:add_item` `project:update_item`, `repo:read`, `ci:read` `ci:rerun`, `release:create` `release:read` `release:publish`, `search:read`, `security:read` `security:dismiss`. `sekimore-relay check` prints the ones a configuration grants.
