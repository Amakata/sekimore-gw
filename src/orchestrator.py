...
class SecurityGatewayOrchestrator:
    ...
    async def initialize(self) -> bool:
        ...
        if self.proxy_manager:
            if not self.proxy_manager.generate_config(self.config.proxy_allow_domains()):
    ...
    async def reload_config(self) -> bool:
        ...
        try:
            ...
            if self.proxy_manager:
                ...
                # the old set. Subtract both: a domain either side relays must not be served here.
                denied = set(self.config.proxy_denied_domains())
                allowed = [
                    d
                    for d in new_config.proxy_allow_domains()
                    if d.lower().rstrip(".") not in denied
                ]
                if not self.proxy_manager.generate_config(allowed):
